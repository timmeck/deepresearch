...
def test_auth_rejects_without_key():
    ...
    with patch("src.web.auth.DEEPRESEARCH_API_KEY", "test-secret-key"):
...
def test_auth_allows_correct_key():
    ...
    with patch("src.web.auth.DEEPRESEARCH_API_KEY", "test-secret-key"):
        ...
        client = TestClient(app, raise_server_exceptions=False)
        resp = client.get("/api/projects", headers={"X-API-Key": "test-secret-key"})
        assert resp.status_code != 401
...
def test_auth_no_query_param_bypass():
    ...
    with patch("src.web.auth.DEEPRESEARCH_API_KEY", "test-secret-key"):
        ...
        client = TestClient(app, raise_server_exceptions=False)
        resp = client.get("/api/projects?key=test-secret-key")
        assert resp.status_code == 401
